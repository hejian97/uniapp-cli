...
function visit$1(root, visitor, visitorKeys = QueryDocumentKeys$1) {
  ...
  do {
    ...
    if (!Array.isArray(node)) {
      ...
      isNode$1(node) || devAssert$1(false, `Invalid AST Node: ${inspect$1(node)}.`);
...
function print$1(ast) {
  return visit$1(ast, printDocASTReducer$1);
}
...
const printDocASTReducer$1 = {
  ...
  SchemaExtension: {
    leave: ({ directives, operationTypes }) => join$1(["extend schema", join$1(directives, " "), block$1(operationTypes)], " ")
  },
  ScalarTypeExtension: {
...
function makeFetchOptions({ query, variables }, opts) {
  const normalizedQuery = normalizeQuery$1(query);
  if (!normalizedQuery) {
...
function getQueryKey(operation, ...components) {
  ...
  const query = normalizeQuery$1(operation.query);
...
function dedup() {
  ...
  return function dedupPlugin(ctx) {
    ...
    let resolveOp;
    pendingLookup[ctx.operation.key] = new Promise((resolve2) => {
      resolveOp = resolve2;
    });
...
var ReactNativeFile_1 = class ReactNativeFile2 {
...
function multipart() {
  return definePlugin(function multipartPlugin(context) {
    ...
    const { files, clone: variables } = extractFiles$1(Object.assign({}, (operation === null || operation === void 0 ? void 0 : operation.variables) || {}));
    if (!files.size) {
      return;
    }
